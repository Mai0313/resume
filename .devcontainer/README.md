...
3. Then `make run` for the Vite dev server, `make build` for a production build, `make fmt` to lint and format.

## Customization

- **Pin a Node version**: pass `{"version": "22"}` to the `node` feature.
